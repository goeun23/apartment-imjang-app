...
export default function RecordDetailPage({
  ...
  const nextPhoto = () => {
    ...
    setCurrentPhotoIndex((prev) =>
      prev === record.record_photos.length - 1 ? 0 : prev + 1
    )
  }
  ...
  const prevPhoto = () => {
    if (!record || !record.record_photos.length) return
    setCurrentPhotoIndex((prev) =>
      prev === 0 ? record.record_photos.length - 1 : prev - 1
    )
  }
  ...
  const sortedPhotos = [...record.record_photos].sort(
          ...
          <div className="flex items-start justify-between mb-4">
            <div>
              ...
              <p className="text-gray-600 mt-1">
                {record.region_si} {record.region_gu} {record.region_dong}{" "}
                {record.address_full}
              </p>
            </div>
